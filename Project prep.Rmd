...
state_data_limited <- data_by_state %>% dplyr::select(Salaries, Region.Code, Service.Population, Circulation.Transactions, Library.Visits, Library.Programs)
...
tree_fit <- tree(Salaries ~ ., data = state_data_limited, split = "deviance")
...
leaflet(data = map_data) %>% 
  ...
  addCircleMarkers(clusterOptions = markerClusterOptions(),radius = ~ifelse(County.Population > 500000, 15, 10), color = "green", stroke = FALSE, fillOpacity = .5, label = ~Library.Name, labelOptions = labelOptions(noHide = T, textsize = "12px"))
```
